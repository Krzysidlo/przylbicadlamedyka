...
<nav class="navbar fixed-top navbar-dark bg-red">
    ...
    <?php if (!$user->noAddress() && USER_PRV === User::USER_PRODUCER) { ?>
        <button class="btn btn-white my-0" data-toggle="modal" data-target="#bascinetModal">Zgłoś gotowe przyłbice
        </button>
        <button class="btn btn-outline-white my-0" data-toggle="modal" data-target="#materialModal">Zgłoś
            zapotrzebowanie
        </button>
    <?php }
    if (!$user->noAddress() && USER_PRV === User::USER_DRIVER) { ?>
        ...
        <button class="btn btn-outline-white my-0" data-toggle="modal" data-target="#collectionModal">Zgłoś odbiór
        </button>
    <?php }
